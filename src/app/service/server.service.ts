...
export class ServerService {
  ...
  getServer$ = (id: number)=><Observable<CustomResponse>>
    this.http.get<CustomResponse>(`${this.apiUrl}/server/${id}`)
      .pipe(
        tap(console.log),
        catchError(this.handleError)
      )
         ...
         status === Status.ALL ? {...response, message: `Servers filtered by ${status} status`} :
           {
             ...
             message: response.data.servers
               ?.filter(server => server.status === status).length > 0 ? `Server filtered by ${status === Status.SERVER_UP ? "SERVER_UP" : "SERVER_DOWN"} status` : `No server of ${status} found`,
